...
def read_host_excel_data(host_excel_file_path, default_password=''):
    ...
    print('aaaaa',host_excel_file_path.getvalue())
    ...
    for row_number in range(1, rows_count):
        ...
        for category_data in category_list:
            ...
            if category_data[1].strip() == category.strip():
                ...
                new_hostcategory_obj = HostCategory.objects.update_or_create(defaults={'name':category.strip(),},
                                                                             name=category.strip())
                one_row_dict['category'] = new_hostcategory_obj[0].id
