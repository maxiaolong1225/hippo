...
class SSHConsumer(WebsocketConsumer):
    ...
    def loop_read(self):
        ...
        while True:
            data = self.chan.recv(32 * 1024)  # 没有指令结果就一直等着
            print('指令结果： ',data)
            if not data:
                self.close(3333)
                break
    ...
    def _init(self):
        ...
        host = models.Hosts.objects.filter(pk=self.id).first()
        if not host:
            self.send(text_data='Unknown host\r\n')
            self.close()  #关闭链接
        try:
            pkey = models.PkeyModel.objects.get(key='private_key').value
            self.ssh = host.get_ssh(pkey).get_client()
        except Exception as e:
            self.send(f'Exception: {e}\r\n')
            self.close()
            return
        #  exec_comand   -- ifconfig
        self.chan = self.ssh.invoke_shell(term='xterm')  # invoke_shell激活shell伪终端模式，也就是长连接模式，exec_command()函数是将服务器执行完的结果一次性返回给你；invoke_shell()函数类似shell终端，可以将执行结果分批次返回，所以我们接受数据时需要循环的取数据
